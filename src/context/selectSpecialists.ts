...
function anyFile(prData: PRData, predicate: (filename: string) => boolean): boolean {
  return prData.files.some((f) => predicate(f.filename));
}

function diffMatches(prData: PRData, rx: RegExp): boolean {
 ...
 */
export function selectSpecialists(prData: PRData): SpecialistName[] {
  const out: SpecialistName[] = [];
