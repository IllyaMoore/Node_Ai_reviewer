...
export async function runReview(config, previousReview) {
    ...
    if (dryRun) {
        ...
        const { posted, dropped } = await submitReview(octokit, owner, repo, prNumber, result.verdict, summaryBody, inline, commentable);
        postSpinner.stop(`${c.green}Posted${c.reset} ${c.dim}(${posted} inline${dropped.length ? `, ${dropped.length} fell back to summary` : ""})${c.reset}`);
    }
...
async function postReviewActions(config, result, _prData, summaryBody, inline, commentable) {
    ...
    if (!canPost && !canReReview)
        return;
    const options = [];
    if (canPost)
        options.push(`${c.yellow}p${c.reset}  Post this review to GitHub`);
    if (canReReview)
        options.push(`${c.yellow}r${c.reset}  Re-review with previous context`);
